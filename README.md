...
- **Calendrier** — vue mensuelle plein écran, doublée d'un suivi d'habitude.
  ...
  déplacer le contour du jour ; il faut rester appuyé 750 ms — un
...
- **Tutoriels par onglet** — une visite guidée (deux à trois étapes selon
  l'écran, une seule pour Profil) encadre les éléments clés de chaque onglet
  à l'aide d'un projecteur et d'une bulle d'explication. Deux sous-écrans de
  l'onglet Ajouter ont la leur en plus de la liste : aliment personnalisé et
  objectif nutritionnel. Elle se déclenche automatiquement la première fois
  qu'on visite un écran, et peut être rouverte à tout moment par le bouton
