  ...
  return (
    <div className="stack">
      ...
      */}
      ...
      {/* Réglage d'affichage, pas de calcul : il n'a rien à faire au-dessus des macros. */}
      <label className="check-row">
        <input
          type="checkbox"
          checked={profile.dayMacrosOpen}
          onChange={(event) => updateProfile({ dayMacrosOpen: event.target.checked })}
        />
        <span>
          {t('profile.dayMacrosOpen')}
          <span className="hint">{t('profile.dayMacrosOpenHint')}</span>
        </span>
      </label>
    </div>
  )
