import type { Lang } from '../lib/types'

/** Le français fait référence : les autres langues doivent en couvrir toutes les clés. */
const fr = {
  'app.name': 'FitnessFufs',
  'app.tagline': 'Calories, macros et poids',

  'nav.diary': 'Journal',
  'nav.add': 'Ajouter',
  'nav.weight': 'Suivi',
  'nav.profile': 'Profil',
  'nav.calendar': 'Calendrier',
  'nav.fridge': 'Frigo',

  'meal.breakfast': 'Petit-déjeuner',
  'meal.lunch': 'Déjeuner',
  'meal.dinner': 'Dîner',
  'meal.snack': 'En-cas',

  'macro.kcal': 'Calories',
  'macro.protein': 'Protéines',
  'macro.carbs': 'Glucides',
  'macro.fat': 'Lipides',
  'macro.protein.short': 'P',
  'macro.carbs.short': 'G',
  'macro.fat.short': 'L',
  'macro.fiber': 'Fibres',
  'macro.fiber.short': 'F',

  'micros.title': 'Micronutriments',
  'micros.none': 'Micronutriments non renseignés pour cet aliment.',
  'micros.partial': 'Calculé sur {n} % des calories du jour : tous les aliments ne sont pas renseignés.',
  'micros.reference': 'des apports de référence',
  'micro.sodium': 'Sodium',
  'micro.potassium': 'Potassium',
  'micro.calcium': 'Calcium',
  'micro.iron': 'Fer',
  'micro.magnesium': 'Magnésium',
  'micro.zinc': 'Zinc',
  'micro.vitaminC': 'Vitamine C',
  'micro.vitaminD': 'Vitamine D',
  'micro.vitaminB12': 'Vitamine B12',

  'diary.remaining': 'Restant',
  'diary.over': 'Dépassement',
  'diary.eaten': 'Consommé',
  'diary.goal': 'Objectif',
  'diary.today': "Aujourd'hui",
  'diary.backToToday': "Revenir à aujourd'hui",
  'diary.prevDay': 'Jour précédent',
  'diary.nextDay': 'Jour suivant',
  'diary.emptyMeal': 'Rien pour ce repas',
  'diary.addFood': 'Ajouter un aliment',
  'diary.moveMealUp': 'Déplacer vers le haut',
  'diary.moveMealDown': 'Déplacer vers le bas',
  'diary.renameMeal': 'Renommer ce repas',
  'diary.markMealDone': 'Marquer ce repas comme terminé',
  'diary.addMeal': 'Ajouter un repas',
  'diary.newMealPlaceholder': 'Nom du repas',
  'diary.noProfile': 'Complétez votre profil pour voir vos objectifs.',
  'diary.copyPrev': 'Copier la veille',
  'diary.copied': 'Journal de la veille copié',
  'diary.nothingToCopy': 'Rien à copier la veille',

  'add.title': 'Ajouter un aliment',
  'add.search': 'Rechercher un aliment…',
  'add.noResults': 'Aucun aliment trouvé',
  'add.createFrom': 'Créer « {q} »',
  'add.per100': 'Pour 100 g',
  'add.quantity': 'Quantité (g)',
  'add.meal': 'Repas',
  'add.confirm': 'Ajouter au journal',
  'add.added': 'Ajouté au journal',
  'add.custom': 'Aliment personnalisé',
  'add.customName': "Nom de l'aliment",
  'add.customHint': 'Valeurs pour 100 g',
  'add.customServing': 'Portion usuelle (g)',
  'add.servingHint': 'Portion usuelle : {n} g',
  'add.kcalMismatch': 'Calories recalculées depuis les macros : {n} kcal',
  'add.kcalWarn':
    "Les calories saisies ({entered} kcal) ne correspondent pas à celles calculées depuis les macros ({computed} kcal). Laissez le champ vide pour utiliser la valeur calculée automatiquement.",

  'recipe.title': 'Recette',
  'recipe.add': 'Ajouter une recette',
  'recipe.subtitle': "Combinez des aliments déjà enregistrés",
  'recipe.empty': 'Ajoutez des aliments pour composer la recette.',
  'recipe.ingredients': 'Ingrédients',
  'recipe.totalWeight': 'Poids total : {n} g',
  'recipe.per100Hint': 'Valeurs recalculées pour 100 g de ce mélange.',
  'recipe.apply': 'Utiliser ces valeurs',
  'recipe.of': 'de',

  'fridge.hint':
    "Les aliments que vous avez sous la main, pour préparer des suggestions de repas selon vos macros restants.",
  'fridge.empty': "Rien pour l'instant. Cherchez un aliment ci-dessus pour l'ajouter ici.",
  'fridge.section.fridge': 'Frigo',
  'fridge.section.pantry': 'Placard',
  'fridge.section.freezer': 'Congélateur',
  'fridge.addTo': 'Ajouter — {section}',

  'chat.title': 'Assistant nutrition',
  'chat.placeholder': 'Écrivez un message…',
  'chat.send': 'Envoyer',
  'chat.empty': "Demandez une idée de repas selon ce qu'il y a au frigo et vos macros restants.",
  'chat.clear': 'Effacer la conversation',
  'chat.clearConfirm': 'Effacer définitivement cette conversation ?',
  'chat.noKey': "Ajoutez votre clé API Anthropic dans le profil pour activer l'assistant.",
  'chat.capped': "Plafond mensuel estimé atteint ({cap} $). L'assistant reprendra le mois prochain.",
  'chat.error': "Impossible de contacter l'assistant. Vérifiez votre connexion ou votre clé API.",
  'chat.thinking': "L'assistant réfléchit…",
  'chat.suggest.plan': "Faites-moi un plan de repas du jour avec ce qu'il y a au frigo.",
  'chat.suggest.remaining': "Que me reste-t-il à manger pour atteindre mes macros aujourd'hui ?",
  'chat.suggest.quick': 'Une idée de repas rapide avec le frigo ?',
  'chat.sendToDiary': 'Envoyer au journal',
  'chat.schedule': 'Planifier',
  'chat.pickDate': 'Choisir une date',
  'chat.mealScheduled': 'Ajouté au journal du {date}',
  'chat.mealScheduledLabel': '{meal} du {date} planifié',
  'chat.mealAdded': 'Repas ajouté au journal',
  'chat.createFood': 'Créer',
  'chat.foodAdded': 'Aliment créé et ajouté au journal',
  'chat.similarFound': '« {name} » ressemble à :',
  'chat.createInstead': 'Créer un nouvel aliment',
  'chat.removeIngredient': "Retirer cet ingrédient",
  'profile.apiKey': 'Clé API Anthropic',
  'profile.apiKeyHint':
    "Utilisée uniquement sur cet appareil, envoyée directement à Anthropic pour l'assistant nutrition. Jamais incluse dans l'export.",
  'profile.usagePeriod': '{pct} % utilisé depuis le {date}',
  'profile.usageHint':
    "Estimation à partir des tokens facturés par l'API, à titre indicatif — ne remplace pas un vrai plafond réglé sur console.anthropic.com. Remise à zéro un mois après le début de cette période.",
  'add.deleteFood': 'Supprimer cet aliment',
  'add.editFood': 'Corriger cet aliment',
  'add.updated': 'Valeurs mises à jour',

  'target.title': 'Objectif nutritionnel',
  'target.subtitle': 'Comparez les aliments d\'une catégorie',
  'target.category': 'Catégorie d\'aliments',
  'target.nutrient': 'Nutriment cible',
  'target.quantity': 'Quantité cible',
  'target.goal': 'Objectif : {qty} {unit} de {nutrient}',
  'target.empty': 'Aucun aliment de cette catégorie n\'apporte ce nutriment.',

  'scan.title': 'Scanner un code-barres',
  'scan.hint': 'Cadrez le code-barres du produit',
  'scan.denied': 'Accès à la caméra refusé. Autorisez-le dans les réglages du navigateur.',
  'scan.unavailable': 'Caméra indisponible sur cet appareil.',
  'scan.notFound': "Produit inconnu d'Open Food Facts. Créez-le à la main.",
  'scan.failed': 'Recherche impossible : vérifiez votre connexion.',
  'off.search': 'Chercher dans Open Food Facts',
  'off.searching': 'Recherche en cours…',
  'off.results': 'Produits trouvés : {n}',
  'off.none': 'Aucun produit trouvé dans Open Food Facts.',
  'off.error': 'Open Food Facts est injoignable. Vérifiez votre connexion et réessayez.',
  'off.source': "Données Open Food Facts, contribuées par ses utilisateurs — à vérifier sur l'emballage.",

  'add.unit': 'Unité',
  'add.results': 'Aliments : {n}',
  'add.totalHint': 'Soit {n} g au total',
  'portion.unit': 'Unité',
  'portion.spoon': 'C. à soupe',
  'portion.teaspoon': 'C. à café',
  'portion.slice': 'Tranche',
  'portion.glass': 'Verre',
  'portion.pot': 'Pot',
  'portion.handful': 'Poignée',
  'portion.dose': 'Dosette',
  'add.category': 'Catégorie',

  'state.title': 'Préparation',
  'state.raw': 'Cru',
  'state.cooked': 'Cuit',

  'cat.all': 'Tous',
  'cat.favorites': 'Favoris',
  'fav.added': "{name} en favori — touchez son nom pour l'ajouter au journal",
  'fav.removed': '{name} retiré des favoris',
  'fav.empty':
    "Aucun favori pour l'instant. L'étoile épingle un aliment en haut de la liste ; elle ne l'ajoute pas au journal — pour cela, touchez son nom.",
  'cat.mine': 'Mes aliments',
  'cat.protein': 'Protéines',
  'cat.carbs': 'Féculents',
  'cat.dairy': 'Laitages',
  'cat.fruit': 'Fruits',
  'cat.veg': 'Légumes',
  'cat.fat': 'Matières grasses',
  'cat.drink': 'Boissons',
  'cat.snack': 'En-cas',
  'cat.dish': 'Plats',
  'cat.supplement': 'Compléments',

  'weight.title': 'Suivi du poids',
  'weight.current': 'Poids actuel',
  'weight.log': 'Enregistrer',
  'weight.input': 'Poids (kg)',
  'weight.empty': 'Aucune pesée enregistrée.',
  'photo.title': 'Photos de progression',
  'photo.empty': 'Aucune photo pour le moment.',
  'photo.add': 'Ajouter une photo',
  'photo.take': 'Prendre une photo',
  'photo.retake': 'Reprendre',
  'photo.saved': 'Photo enregistrée',
  'photo.deleted': 'Photo supprimée',
  'photo.deleteConfirm': 'Supprimer cette photo ?',
  'photo.error': "Impossible d'enregistrer cette photo",
  'weight.bmi': 'IMC',
  'weight.history': 'Historique du poids',
  'weight.syncProfile': 'Le profil utilise toujours la dernière pesée.',
  'weight.trend': 'Évolution',
  'chart.range1m': '1 M',
  'chart.range3m': '3 M',
  'chart.range6m': '6 M',
  'chart.range1y': '1 A',
  'chart.rangeAll': 'Tout',
  'chart.notEnough': 'Pas assez de données sur cette période.',
  'measure.title': 'Mesures corporelles',
  'measure.waist': 'Tour de taille',
  'measure.hips': 'Tour de hanches',
  'measure.chest': 'Tour de poitrine',
  'measure.log': 'Ajouter nouvelle mesure',
  'measure.logged': 'Mesures enregistrées',
  'measure.empty': 'Aucune mesure enregistrée.',
  'measure.history': 'Historique des mesures',
  'history.title': 'Historique',
  'history.open': "Voir l'historique",

  'bmi.under': 'Insuffisance pondérale',
  'bmi.normal': 'Corpulence normale',
  'bmi.over': 'Surpoids',
  'bmi.obese': 'Obésité',
  'bmi.morbid': 'Obésité morbide',

  'account.title': 'Compte',
  'account.loading': 'Chargement…',
  'account.connectedAs': 'Connecté en tant que',
  'account.signOut': 'Se déconnecter',
  'account.signIn': 'Se connecter',
  'account.signUp': 'Créer un compte',
  'account.email': 'E-mail',
  'account.password': 'Mot de passe',
  'account.rememberMe': 'Se souvenir de moi',
  'account.continueWithGoogle': 'Continuer avec Google',
  'account.confirmEmail': 'Vérifiez votre boîte mail pour confirmer votre compte.',
  'account.greeting': 'Content de vous revoir, {name} !',
  'account.skip': 'Continuer sans compte',
  'account.syncError': 'Échec de la synchronisation — nouvelle tentative au prochain changement.',
  'profile.title': 'Profil et objectifs',
  'profile.displayName': "Comment souhaitez-vous qu'on vous appelle ?",
  'profile.height': 'Taille (cm)',
  'profile.weight': 'Poids (kg)',
  'profile.age': 'Âge',
  'profile.sex': 'Sexe',
  'profile.male': 'Homme',
  'profile.female': 'Femme',
  'profile.activity': "Niveau d'activité",
  'profile.goal': 'Objectif',
  'profile.targets': 'Objectifs quotidiens',
  'profile.preset': 'Programme',
  'preset.balanced': 'Équilibré',
  'preset.highprotein': 'Riche en protéines',
  'preset.lowcarb': 'Pauvre en glucides',
  'preset.keto': 'Cétogène',
  'preset.endurance': 'Endurance',
  'preset.custom': 'Personnalisé',
  'profile.carbsAuto': "Les glucides complètent automatiquement l'objectif calorique.",
  'day.macros': 'Macros du jour',
  'day.custom': 'personnalisées',
  'day.reset': 'Revenir aux macros du profil',
  'day.followsProfile': 'Cette journée suit le profil. Bouger un curseur ne change que ce jour-là.',
  'day.noteTitle': 'Note du jour',
  'day.notePlaceholder': 'Repas sauté, entraînement, voyage…',
  'day.noteSave': 'Enregistrer',
  'day.noteDelete': 'Supprimer la note',
  'day.noteHoldHint': "Astuce : rester appuyé sur un jour l'ouvre directement dans le journal.",
  'calendar.streakCalories': 'Calories',
  'calendar.streakMacros': 'Macros',
  'profile.dayMacrosOpen': 'Ouvrir les macros du jour dans le journal',
  'profile.dayMacrosOpenHint':
    "Le panneau reste déplié sur chaque journée au lieu d'être replié derrière sa flèche.",
  'profile.warnMacros':
    "Protéines et lipides pèsent {n} kcal de plus que l'objectif : les glucides restent à zéro. Baissez un curseur ou relevez l'objectif pour les voir remonter.",
  'profile.bmr': 'Métabolisme de base',
  'profile.tdee': 'Dépense estimée',
  'profile.adjustment': 'Ajustement',
  'profile.requested': 'Objectif demandé',
  'profile.warnBmr':
    "Objectif demandé : {req} kcal, sous votre métabolisme de base. La valeur retenue est relevée à {n} kcal ; tant qu'on reste sous ce plancher, changer le poids, l'âge, l'activité ou l'objectif ne fait bouger que la valeur demandée.",
  'profile.language': 'Langue',
  'profile.data': 'Données',
  'profile.export': 'Exporter (JSON)',
  'profile.import': 'Importer',
  'profile.reset': 'Tout effacer',
  'profile.resetConfirm': 'Effacer définitivement toutes vos données ?',
  'profile.importError': 'Fichier invalide',
  'profile.imported': 'Données importées',
  'profile.offline': 'Toutes vos données restent sur cet appareil.',
  'profile.tutorials': 'Tutoriels automatiques',
  'profile.tutorialsHint':
    "Affiche une visite guidée au premier passage sur chaque onglet. Le bouton « ? » de l'en-tête reste toujours disponible.",

  'act.sedentary': 'Sédentaire (bureau)',
  'act.light': 'Léger (1-3× / semaine)',
  'act.moderate': 'Modéré (3-5× / semaine)',
  'act.active': 'Très actif (6-7× / semaine)',
  'act.athlete': 'Athlète (2× / jour)',

  'goal.loss150': 'Perte : 1,50 kg / semaine',
  'goal.loss100': 'Perte : 1,00 kg / semaine',
  'goal.loss075': 'Perte : 0,75 kg / semaine',
  'goal.loss050': 'Perte : 0,50 kg / semaine',
  'goal.loss025': 'Perte : 0,25 kg / semaine',
  'goal.maintain': 'Maintien du poids',
  'goal.gain025': 'Prise : +0,25 kg / semaine',
  'goal.gain050': 'Prise : +0,50 kg / semaine',

  'onboarding.welcome': 'Bienvenue sur FitnessFufs',
  'onboarding.intro':
    'Quelques informations suffisent pour calculer vos objectifs quotidiens. Tout reste stocké sur votre téléphone.',
  'onboarding.start': 'Commencer',
  'onboarding.skip': 'Plus tard',

  'tour.help': 'Aide',
  'tour.skip': 'Passer',
  'tour.next': 'Suivant',
  'tour.done': 'Terminé',
  'tour.calendar.1':
    "Touchez un jour pour le sélectionner ; restez appuyé pour ouvrir directement son journal. Balayez horizontalement pour changer de mois.",
  'tour.calendar.2':
    "Chaque barre montre l'écart par rapport à votre objectif : verte si proche, rouge si loin, dans les deux sens.",
  'tour.diary.1':
    "Changez de jour avec les flèches, ou balayez à deux doigts pour ne pas interférer avec le défilement de la page.",
  'tour.diary.2': 'Votre progression calorique du jour, mise à jour à chaque aliment ajouté.',
  'tour.diary.3': 'Ajoutez une note libre à cette journée — entraînement, voyage, repas sauté.',
  'tour.add.1': 'Cherchez un aliment, scannez un code-barres, ou créez le vôtre.',
  'tour.add.2': "Comparez les aliments pour atteindre une quantité cible d'un nutriment.",
  'tour.weight.1':
    "L'évolution de votre poids, avec un sélecteur de période pour zoomer sur les dernières semaines ou tout l'historique.",
  'tour.weight.2':
    "Votre IMC, positionné sur une jauge colorée pour voir en un coup d'œil où vous vous situez.",
  'tour.fridge.1': "Cherchez un aliment et touchez-le pour l'ajouter au frigo avec son poids.",
  'tour.fridge.2':
    'Ajustez le poids disponible à tout moment, ou retirez un aliment une fois utilisé.',
  'tour.profile.1':
    "Retrouvez ici comment couper ou réactiver le déclenchement automatique des tutoriels.",
  'tour.addCustom.1': "Le nom de l'aliment, tel qu'il apparaîtra dans vos recherches.",
  'tour.addCustom.2':
    "Composez-le à partir d'aliments déjà enregistrés : la recette calcule automatiquement les macros et micros proportionnels.",
  'tour.addCustom.3':
    'Indiquez les calories, ou laissez-les vides : elles seront recalculées depuis les macros renseignées plus bas.',
  'tour.addTarget.1':
    "Comparez les aliments d'une catégorie donnée, ou de toutes les catégories à la fois.",
  'tour.addTarget.2': 'Choisissez le nutriment visé et la quantité à atteindre.',
  'tour.addTarget.3':
    "Chaque résultat indique le poids nécessaire pour atteindre votre objectif ; touchez-le pour l'ajouter directement au journal.",

  'common.save': 'Enregistrer',
  'common.cancel': 'Annuler',
  'common.delete': 'Supprimer',
  'common.close': 'Fermer',
  'common.deficit': 'Déficit',
  'common.surplus': 'Surplus',
  'common.none': 'Aucun',
  'common.install': "Installer l'application",
  'common.update': 'Mise à jour disponible',
  'common.refresh': 'Recharger',
  'common.left': 'restant',
} as const

export type TranslationKey = keyof typeof fr
type Dict = Record<TranslationKey, string>

const pt: Dict = {
  'app.name': 'FitnessFufs',
  'app.tagline': 'Calorias, macros e peso',

  'nav.diary': 'Diário',
  'nav.add': 'Adicionar',
  'nav.weight': 'Registro',
  'nav.profile': 'Perfil',
  'nav.calendar': 'Calendário',
  'nav.fridge': 'Geladeira',

  'meal.breakfast': 'Café da manhã',
  'meal.lunch': 'Almoço',
  'meal.dinner': 'Jantar',
  'meal.snack': 'Lanche',

  'macro.kcal': 'Calorias',
  'macro.protein': 'Proteínas',
  'macro.carbs': 'Carboidratos',
  'macro.fat': 'Gorduras',
  'macro.protein.short': 'P',
  'macro.carbs.short': 'C',
  'macro.fat.short': 'G',
  'macro.fiber': 'Fibras',
  'macro.fiber.short': 'F',

  'micros.title': 'Micronutrientes',
  'micros.none': 'Micronutrientes não disponíveis para este alimento.',
  'micros.partial': 'Calculado sobre {n} % das calorias do dia: nem todos os alimentos estão preenchidos.',
  'micros.reference': 'dos valores de referência',
  'micro.sodium': 'Sódio',
  'micro.potassium': 'Potássio',
  'micro.calcium': 'Cálcio',
  'micro.iron': 'Ferro',
  'micro.magnesium': 'Magnésio',
  'micro.zinc': 'Zinco',
  'micro.vitaminC': 'Vitamina C',
  'micro.vitaminD': 'Vitamina D',
  'micro.vitaminB12': 'Vitamina B12',

  'diary.remaining': 'Restante',
  'diary.over': 'Excedido',
  'diary.eaten': 'Consumido',
  'diary.goal': 'Meta',
  'diary.today': 'Hoje',
  'diary.backToToday': 'Voltar a hoje',
  'diary.prevDay': 'Dia anterior',
  'diary.nextDay': 'Dia seguinte',
  'diary.emptyMeal': 'Nada nesta refeição',
  'diary.addFood': 'Adicionar alimento',
  'diary.moveMealUp': 'Mover para cima',
  'diary.moveMealDown': 'Mover para baixo',
  'diary.renameMeal': 'Renomear esta refeição',
  'diary.markMealDone': 'Marcar esta refeição como concluída',
  'diary.addMeal': 'Adicionar refeição',
  'diary.newMealPlaceholder': 'Nome da refeição',
  'diary.noProfile': 'Complete o seu perfil para ver as metas.',
  'diary.copyPrev': 'Copiar dia anterior',
  'diary.copied': 'Diário do dia anterior copiado',
  'diary.nothingToCopy': 'Nada para copiar do dia anterior',

  'add.title': 'Adicionar alimento',
  'add.search': 'Procurar alimento…',
  'add.noResults': 'Nenhum alimento encontrado',
  'add.createFrom': 'Criar «{q}»',
  'add.per100': 'Por 100 g',
  'add.quantity': 'Quantidade (g)',
  'add.meal': 'Refeição',
  'add.confirm': 'Adicionar ao diário',
  'add.added': 'Adicionado ao diário',
  'add.custom': 'Alimento personalizado',
  'add.customName': 'Nome do alimento',
  'add.customHint': 'Valores por 100 g',
  'add.customServing': 'Porção habitual (g)',
  'add.servingHint': 'Porção habitual: {n} g',
  'add.kcalMismatch': 'Calorias recalculadas a partir dos macros: {n} kcal',
  'add.kcalWarn':
    'As calorias inseridas ({entered} kcal) não correspondem às calculadas a partir dos macros ({computed} kcal). Deixe o campo vazio para usar o valor calculado automaticamente.',

  'recipe.title': 'Receita',
  'recipe.add': 'Adicionar uma receita',
  'recipe.subtitle': 'Combine alimentos já registrados',
  'recipe.empty': 'Adicione alimentos para compor a receita.',
  'recipe.ingredients': 'Ingredientes',
  'recipe.totalWeight': 'Peso total: {n} g',
  'recipe.per100Hint': 'Valores recalculados para 100 g desta mistura.',
  'recipe.apply': 'Usar estes valores',
  'recipe.of': 'de',

  'fridge.hint':
    'Os alimentos que você tem à mão, para preparar sugestões de refeição de acordo com as macros restantes.',
  'fridge.empty': 'Nada por aqui ainda. Busque um alimento acima para adicioná-lo.',
  'fridge.section.fridge': 'Geladeira',
  'fridge.section.pantry': 'Armário',
  'fridge.section.freezer': 'Freezer',
  'fridge.addTo': 'Adicionar — {section}',

  'chat.title': 'Assistente de nutrição',
  'chat.placeholder': 'Escreva uma mensagem…',
  'chat.send': 'Enviar',
  'chat.empty': 'Peça uma ideia de refeição de acordo com o que há na geladeira e suas macros restantes.',
  'chat.clear': 'Apagar a conversa',
  'chat.clearConfirm': 'Apagar esta conversa definitivamente?',
  'chat.noKey': 'Adicione sua chave de API da Anthropic no perfil para ativar o assistente.',
  'chat.capped': 'Limite mensal estimado atingido ({cap} $). O assistente volta no mês que vem.',
  'chat.error': 'Não foi possível contatar o assistente. Verifique sua conexão ou sua chave de API.',
  'chat.thinking': 'O assistente está pensando…',
  'chat.suggest.plan': 'Monte um plano de refeições do dia com o que há na geladeira.',
  'chat.suggest.remaining': 'O que ainda posso comer hoje para atingir minhas macros?',
  'chat.suggest.quick': 'Uma ideia de refeição rápida com a geladeira?',
  'chat.sendToDiary': 'Enviar ao diário',
  'chat.schedule': 'Planejar',
  'chat.pickDate': 'Escolher uma data',
  'chat.mealScheduled': 'Adicionado ao diário de {date}',
  'chat.mealScheduledLabel': '{meal} de {date} planejado',
  'chat.mealAdded': 'Refeição adicionada ao diário',
  'chat.createFood': 'Criar',
  'chat.foodAdded': 'Alimento criado e adicionado ao diário',
  'chat.similarFound': '"{name}" parece com:',
  'chat.createInstead': 'Criar um novo alimento',
  'chat.removeIngredient': 'Remover este ingrediente',
  'profile.apiKey': 'Chave de API da Anthropic',
  'profile.apiKeyHint':
    'Usada apenas neste aparelho, enviada diretamente à Anthropic para o assistente de nutrição. Nunca incluída na exportação.',
  'profile.usagePeriod': '{pct} % usado desde {date}',
  'profile.usageHint':
    'Estimativa a partir dos tokens cobrados pela API, apenas indicativa — não substitui um limite real definido em console.anthropic.com. Zerado um mês após o início deste período.',
  'add.deleteFood': 'Eliminar este alimento',
  'add.editFood': 'Corrigir este alimento',
  'add.updated': 'Valores atualizados',

  'target.title': 'Meta nutricional',
  'target.subtitle': 'Compare os alimentos de uma categoria',
  'target.category': 'Categoria de alimentos',
  'target.nutrient': 'Nutriente alvo',
  'target.quantity': 'Quantidade alvo',
  'target.goal': 'Meta: {qty} {unit} de {nutrient}',
  'target.empty': 'Nenhum alimento desta categoria fornece este nutriente.',

  'scan.title': 'Ler código de barras',
  'scan.hint': 'Enquadre o código de barras do produto',
  'scan.denied': 'Acesso à câmara recusado. Autorize nas definições do navegador.',
  'scan.unavailable': 'Câmara indisponível neste dispositivo.',
  'scan.notFound': 'Produto desconhecido no Open Food Facts. Crie-o manualmente.',
  'scan.failed': 'Pesquisa impossível: verifique a sua ligação.',
  'off.search': 'Procurar no Open Food Facts',
  'off.searching': 'A procurar…',
  'off.results': 'Produtos encontrados: {n}',
  'off.none': 'Nenhum produto encontrado no Open Food Facts.',
  'off.error': 'Open Food Facts está inacessível. Verifique a ligação e tente novamente.',
  'off.source': 'Dados do Open Food Facts, contribuídos pelos utilizadores — confirme na embalagem.',

  'add.unit': 'Unidade',
  'add.results': 'Alimentos: {n}',
  'add.totalHint': 'Ou seja, {n} g no total',
  'portion.unit': 'Unidade',
  'portion.spoon': 'Colher de sopa',
  'portion.teaspoon': 'Colher de chá',
  'portion.slice': 'Fatia',
  'portion.glass': 'Copo',
  'portion.pot': 'Pote',
  'portion.handful': 'Punhado',
  'portion.dose': 'Doseador',
  'add.category': 'Categoria',

  'state.title': 'Preparação',
  'state.raw': 'Cru',
  'state.cooked': 'Cozido',

  'cat.all': 'Todos',
  'cat.favorites': 'Favoritos',
  'fav.added': '{name} nos favoritos — toque no nome para adicionar ao diário',
  'fav.removed': '{name} removido dos favoritos',
  'fav.empty':
    'Ainda não há favoritos. A estrela fixa um alimento no topo da lista; não o adiciona ao diário — para isso, toque no nome.',
  'cat.mine': 'Os meus alimentos',
  'cat.protein': 'Proteínas',
  'cat.carbs': 'Carboidratos',
  'cat.dairy': 'Laticínios',
  'cat.fruit': 'Fruta',
  'cat.veg': 'Legumes',
  'cat.fat': 'Gorduras',
  'cat.drink': 'Bebidas',
  'cat.snack': 'Snacks',
  'cat.dish': 'Pratos',
  'cat.supplement': 'Suplementos',

  'weight.title': 'Registro de peso',
  'weight.current': 'Peso atual',
  'weight.log': 'Registrar',
  'weight.input': 'Peso (kg)',
  'weight.empty': 'Nenhuma pesagem registrada.',
  'photo.title': 'Fotos de progresso',
  'photo.empty': 'Nenhuma foto por enquanto.',
  'photo.add': 'Adicionar uma foto',
  'photo.take': 'Tirar uma foto',
  'photo.retake': 'Tirar de novo',
  'photo.saved': 'Foto salva',
  'photo.deleted': 'Foto excluída',
  'photo.deleteConfirm': 'Excluir esta foto?',
  'photo.error': 'Não foi possível salvar esta foto',
  'weight.bmi': 'IMC',
  'weight.history': 'Histórico de peso',
  'weight.syncProfile': 'O perfil usa sempre a última pesagem.',
  'weight.trend': 'Evolução',
  'chart.range1m': '1 M',
  'chart.range3m': '3 M',
  'chart.range6m': '6 M',
  'chart.range1y': '1 A',
  'chart.rangeAll': 'Tudo',
  'chart.notEnough': 'Não há dados suficientes neste período.',
  'measure.title': 'Medidas corporais',
  'measure.waist': 'Cintura',
  'measure.hips': 'Anca',
  'measure.chest': 'Peito',
  'measure.log': 'Adicionar nova medida',
  'measure.logged': 'Medidas salvas',
  'measure.empty': 'Nenhuma medida registrada.',
  'measure.history': 'Histórico de medidas',
  'history.title': 'Histórico',
  'history.open': 'Ver histórico',

  'bmi.under': 'Peso abaixo do normal',
  'bmi.normal': 'Peso normal',
  'bmi.over': 'Excesso de peso',
  'bmi.obese': 'Obesidade',
  'bmi.morbid': 'Obesidade mórbida',

  'account.title': 'Conta',
  'account.loading': 'Carregando…',
  'account.connectedAs': 'Conectado como',
  'account.signOut': 'Sair',
  'account.signIn': 'Entrar',
  'account.signUp': 'Criar conta',
  'account.email': 'E-mail',
  'account.password': 'Senha',
  'account.rememberMe': 'Lembrar de mim',
  'account.continueWithGoogle': 'Continuar com o Google',
  'account.confirmEmail': 'Verifique seu e-mail para confirmar sua conta.',
  'account.greeting': 'Que bom te ver de novo, {name}!',
  'account.skip': 'Continuar sem conta',
  'account.syncError': 'Falha na sincronização — nova tentativa na próxima alteração.',
  'profile.title': 'Perfil e metas',
  'profile.displayName': 'Como prefere ser chamado?',
  'profile.height': 'Altura (cm)',
  'profile.weight': 'Peso (kg)',
  'profile.age': 'Idade',
  'profile.sex': 'Sexo',
  'profile.male': 'Homem',
  'profile.female': 'Mulher',
  'profile.activity': 'Nível de atividade',
  'profile.goal': 'Objetivo',
  'profile.targets': 'Metas diárias',
  'profile.preset': 'Programa',
  'preset.balanced': 'Equilibrado',
  'preset.highprotein': 'Rico em proteína',
  'preset.lowcarb': 'Baixo em hidratos',
  'preset.keto': 'Cetogénico',
  'preset.endurance': 'Resistência',
  'preset.custom': 'Personalizado',
  'profile.carbsAuto': 'Os hidratos completam automaticamente a meta calórica.',
  'day.macros': 'Macros do dia',
  'day.custom': 'personalizadas',
  'day.reset': 'Voltar às macros do perfil',
  'day.followsProfile': 'Este dia segue o perfil. Mexer num cursor altera apenas este dia.',
  'day.noteTitle': 'Nota do dia',
  'day.notePlaceholder': 'Refeição saltada, treino, viagem…',
  'day.noteSave': 'Salvar',
  'day.noteDelete': 'Eliminar a nota',
  'day.noteHoldHint': 'Dica: manter o dedo pressionado num dia abre-o diretamente no diário.',
  'calendar.streakCalories': 'Calorias',
  'calendar.streakMacros': 'Macros',
  'profile.dayMacrosOpen': 'Abrir as macros do dia no diário',
  'profile.dayMacrosOpenHint':
    'O painel fica aberto em todos os dias em vez de ficar recolhido atrás da seta.',
  'profile.warnMacros':
    'As proteínas e as gorduras pesam mais {n} kcal do que a meta: os hidratos ficam a zero. Baixe um cursor ou aumente a meta para os ver subir.',
  'profile.bmr': 'Metabolismo basal',
  'profile.tdee': 'Gasto estimado',
  'profile.adjustment': 'Ajuste',
  'profile.requested': 'Meta pedida',
  'profile.warnBmr':
    'Meta pedida: {req} kcal, abaixo do metabolismo basal. O valor retido é elevado para {n} kcal; enquanto ficar abaixo deste piso, mudar o peso, a idade, a atividade ou o objetivo só altera o valor pedido.',
  'profile.language': 'Idioma',
  'profile.data': 'Dados',
  'profile.export': 'Exportar (JSON)',
  'profile.import': 'Importar',
  'profile.reset': 'Apagar tudo',
  'profile.resetConfirm': 'Apagar definitivamente todos os seus dados?',
  'profile.importError': 'Arquivo inválido',
  'profile.imported': 'Dados importados',
  'profile.offline': 'Todos os dados ficam neste dispositivo.',
  'profile.tutorials': 'Tutoriais automáticos',
  'profile.tutorialsHint':
    'Mostra um tour guiado na primeira vez que você acessa cada aba. O botão "?" do cabeçalho continua sempre disponível.',

  'act.sedentary': 'Sedentário (escritório)',
  'act.light': 'Leve (1-3× / semana)',
  'act.moderate': 'Moderado (3-5× / semana)',
  'act.active': 'Muito ativo (6-7× / semana)',
  'act.athlete': 'Atleta (2× / dia)',

  'goal.loss150': 'Perda: 1,50 kg / semana',
  'goal.loss100': 'Perda: 1,00 kg / semana',
  'goal.loss075': 'Perda: 0,75 kg / semana',
  'goal.loss050': 'Perda: 0,50 kg / semana',
  'goal.loss025': 'Perda: 0,25 kg / semana',
  'goal.maintain': 'Manutenção do peso',
  'goal.gain025': 'Ganho: +0,25 kg / semana',
  'goal.gain050': 'Ganho: +0,50 kg / semana',

  'onboarding.welcome': 'Bem-vindo ao FitnessFufs',
  'onboarding.intro':
    'Bastam algumas informações para calcular as suas metas diárias. Tudo fica guardado no seu celular.',
  'onboarding.start': 'Começar',
  'onboarding.skip': 'Mais tarde',

  'tour.help': 'Ajuda',
  'tour.skip': 'Pular',
  'tour.next': 'Próximo',
  'tour.done': 'Concluído',
  'tour.calendar.1':
    'Toque em um dia para selecioná-lo; mantenha pressionado para abrir o diário diretamente. Deslize horizontalmente para trocar de mês.',
  'tour.calendar.2':
    'Cada barra mostra o desvio em relação à sua meta: verde se está perto, vermelha se está longe, nos dois sentidos.',
  'tour.diary.1':
    'Troque de dia com as setas, ou deslize com dois dedos para não interferir na rolagem da página.',
  'tour.diary.2': 'Seu progresso calórico do dia, atualizado a cada alimento adicionado.',
  'tour.diary.3': 'Adicione uma nota livre a este dia — treino, viagem, refeição pulada.',
  'tour.add.1': 'Busque um alimento, escaneie um código de barras ou crie o seu.',
  'tour.add.2': 'Compare alimentos para atingir uma quantidade alvo de um nutriente.',
  'tour.weight.1':
    'A evolução do seu peso, com um seletor de período para focar nas últimas semanas ou em todo o histórico.',
  'tour.weight.2':
    'Seu IMC, posicionado em uma régua colorida para ver rapidamente onde você está.',
  'tour.fridge.1': 'Busque um alimento e toque nele para adicioná-lo à geladeira com o peso.',
  'tour.fridge.2':
    'Ajuste o peso disponível a qualquer momento, ou remova um alimento depois de usá-lo.',
  'tour.profile.1':
    'Aqui você encontra como desativar ou reativar o disparo automático dos tutoriais.',
  'tour.addCustom.1': 'O nome do alimento, como ele aparecerá nas suas buscas.',
  'tour.addCustom.2':
    'Monte-o a partir de alimentos já cadastrados: a receita calcula automaticamente as macros e os micros proporcionais.',
  'tour.addCustom.3':
    'Informe as calorias, ou deixe em branco: elas serão recalculadas a partir das macros preenchidas abaixo.',
  'tour.addTarget.1':
    'Compare os alimentos de uma categoria específica, ou de todas as categorias ao mesmo tempo.',
  'tour.addTarget.2': 'Escolha o nutriente desejado e a quantidade a atingir.',
  'tour.addTarget.3':
    'Cada resultado mostra o peso necessário para atingir sua meta; toque nele para adicioná-lo diretamente ao diário.',

  'common.save': 'Salvar',
  'common.cancel': 'Cancelar',
  'common.delete': 'Eliminar',
  'common.close': 'Fechar',
  'common.deficit': 'Défice',
  'common.surplus': 'Superavit',
  'common.none': 'Nenhum',
  'common.install': 'Instalar a aplicação',
  'common.update': 'Atualização disponível',
  'common.refresh': 'Recarregar',
  'common.left': 'restante',
}

const es: Dict = {
  'app.name': 'FitnessFufs',
  'app.tagline': 'Calorías, macros y peso',

  'nav.diary': 'Diario',
  'nav.add': 'Añadir',
  'nav.weight': 'Seguimiento',
  'nav.profile': 'Perfil',
  'nav.calendar': 'Calendario',
  'nav.fridge': 'Nevera',

  'meal.breakfast': 'Desayuno',
  'meal.lunch': 'Comida',
  'meal.dinner': 'Cena',
  'meal.snack': 'Tentempié',

  'macro.kcal': 'Calorías',
  'macro.protein': 'Proteínas',
  'macro.carbs': 'Carbohidratos',
  'macro.fat': 'Grasas',
  'macro.protein.short': 'P',
  'macro.carbs.short': 'C',
  'macro.fat.short': 'G',
  'macro.fiber': 'Fibra',
  'macro.fiber.short': 'F',

  'micros.title': 'Micronutrientes',
  'micros.none': 'Micronutrientes no disponibles para este alimento.',
  'micros.partial': 'Calculado sobre el {n} % de las calorías del día: no todos los alimentos están cubiertos.',
  'micros.reference': 'de los valores de referencia',
  'micro.sodium': 'Sodio',
  'micro.potassium': 'Potasio',
  'micro.calcium': 'Calcio',
  'micro.iron': 'Hierro',
  'micro.magnesium': 'Magnesio',
  'micro.zinc': 'Zinc',
  'micro.vitaminC': 'Vitamina C',
  'micro.vitaminD': 'Vitamina D',
  'micro.vitaminB12': 'Vitamina B12',

  'diary.remaining': 'Restante',
  'diary.over': 'Excedido',
  'diary.eaten': 'Consumido',
  'diary.goal': 'Objetivo',
  'diary.today': 'Hoy',
  'diary.backToToday': 'Volver a hoy',
  'diary.prevDay': 'Día anterior',
  'diary.nextDay': 'Día siguiente',
  'diary.emptyMeal': 'Nada en esta comida',
  'diary.addFood': 'Añadir alimento',
  'diary.moveMealUp': 'Mover hacia arriba',
  'diary.moveMealDown': 'Mover hacia abajo',
  'diary.renameMeal': 'Renombrar esta comida',
  'diary.markMealDone': 'Marcar esta comida como terminada',
  'diary.addMeal': 'Añadir comida',
  'diary.newMealPlaceholder': 'Nombre de la comida',
  'diary.noProfile': 'Completa tu perfil para ver tus objetivos.',
  'diary.copyPrev': 'Copiar día anterior',
  'diary.copied': 'Diario del día anterior copiado',
  'diary.nothingToCopy': 'Nada que copiar del día anterior',

  'add.title': 'Añadir alimento',
  'add.search': 'Buscar un alimento…',
  'add.noResults': 'No se encontró ningún alimento',
  'add.createFrom': 'Crear «{q}»',
  'add.per100': 'Por 100 g',
  'add.quantity': 'Cantidad (g)',
  'add.meal': 'Comida',
  'add.confirm': 'Añadir al diario',
  'add.added': 'Añadido al diario',
  'add.custom': 'Alimento personalizado',
  'add.customName': 'Nombre del alimento',
  'add.customHint': 'Valores por 100 g',
  'add.customServing': 'Ración habitual (g)',
  'add.servingHint': 'Ración habitual: {n} g',
  'add.kcalMismatch': 'Calorías recalculadas desde los macros: {n} kcal',
  'add.kcalWarn':
    'Las calorías introducidas ({entered} kcal) no coinciden con las calculadas a partir de los macros ({computed} kcal). Deja el campo vacío para usar el valor calculado automáticamente.',

  'recipe.title': 'Receta',
  'recipe.add': 'Añadir una receta',
  'recipe.subtitle': 'Combina alimentos ya registrados',
  'recipe.empty': 'Añade alimentos para componer la receta.',
  'recipe.ingredients': 'Ingredientes',
  'recipe.totalWeight': 'Peso total: {n} g',
  'recipe.per100Hint': 'Valores recalculados para 100 g de esta mezcla.',
  'recipe.apply': 'Usar estos valores',
  'recipe.of': 'de',

  'fridge.hint':
    'Los alimentos que tienes a mano, para preparar sugerencias de comida según tus macros restantes.',
  'fridge.empty': 'Nada por aquí todavía. Busca un alimento arriba para añadirlo.',
  'fridge.section.fridge': 'Nevera',
  'fridge.section.pantry': 'Armario',
  'fridge.section.freezer': 'Congelador',
  'fridge.addTo': 'Añadir — {section}',

  'chat.title': 'Asistente de nutrición',
  'chat.placeholder': 'Escribe un mensaje…',
  'chat.send': 'Enviar',
  'chat.empty': 'Pide una idea de comida según lo que hay en la nevera y tus macros restantes.',
  'chat.clear': 'Borrar la conversación',
  'chat.clearConfirm': '¿Borrar definitivamente esta conversación?',
  'chat.noKey': 'Añade tu clave de API de Anthropic en el perfil para activar el asistente.',
  'chat.capped': 'Límite mensual estimado alcanzado ({cap} $). El asistente vuelve el mes que viene.',
  'chat.error': 'No se pudo contactar con el asistente. Comprueba tu conexión o tu clave de API.',
  'chat.thinking': 'El asistente está pensando…',
  'chat.suggest.plan': 'Prepárame un plan de comidas del día con lo que hay en la nevera.',
  'chat.suggest.remaining': '¿Qué me queda por comer hoy para alcanzar mis macros?',
  'chat.suggest.quick': '¿Una idea de comida rápida con la nevera?',
  'chat.sendToDiary': 'Enviar al diario',
  'chat.schedule': 'Planificar',
  'chat.pickDate': 'Elegir una fecha',
  'chat.mealScheduled': 'Añadido al diario del {date}',
  'chat.mealScheduledLabel': '{meal} del {date} planificado',
  'chat.mealAdded': 'Comida añadida al diario',
  'chat.createFood': 'Crear',
  'chat.foodAdded': 'Alimento creado y añadido al diario',
  'chat.similarFound': '"{name}" se parece a:',
  'chat.createInstead': 'Crear un nuevo alimento',
  'chat.removeIngredient': 'Quitar este ingrediente',
  'profile.apiKey': 'Clave de API de Anthropic',
  'profile.apiKeyHint':
    'Se usa solo en este dispositivo, enviada directamente a Anthropic para el asistente de nutrición. Nunca incluida en la exportación.',
  'profile.usagePeriod': '{pct} % usado desde el {date}',
  'profile.usageHint':
    'Estimación a partir de los tokens facturados por la API, solo orientativa — no sustituye a un límite real configurado en console.anthropic.com. Se reinicia un mes después del inicio de este período.',
  'add.deleteFood': 'Eliminar este alimento',
  'add.editFood': 'Corregir este alimento',
  'add.updated': 'Valores actualizados',

  'target.title': 'Objetivo nutricional',
  'target.subtitle': 'Compara los alimentos de una categoría',
  'target.category': 'Categoría de alimentos',
  'target.nutrient': 'Nutriente objetivo',
  'target.quantity': 'Cantidad objetivo',
  'target.goal': 'Objetivo: {qty} {unit} de {nutrient}',
  'target.empty': 'Ningún alimento de esta categoría aporta este nutriente.',

  'scan.title': 'Escanear código de barras',
  'scan.hint': 'Encuadra el código de barras del producto',
  'scan.denied': 'Acceso a la cámara denegado. Autorízalo en los ajustes del navegador.',
  'scan.unavailable': 'Cámara no disponible en este dispositivo.',
  'scan.notFound': 'Producto desconocido en Open Food Facts. Créalo a mano.',
  'scan.failed': 'Búsqueda imposible: comprueba tu conexión.',
  'off.search': 'Buscar en Open Food Facts',
  'off.searching': 'Buscando…',
  'off.results': 'Productos encontrados: {n}',
  'off.none': 'Ningún producto encontrado en Open Food Facts.',
  'off.error': 'Open Food Facts no responde. Comprueba tu conexión e inténtalo de nuevo.',
  'off.source': 'Datos de Open Food Facts, aportados por sus usuarios — verifícalos en el envase.',

  'add.unit': 'Unidad',
  'add.results': 'Alimentos: {n}',
  'add.totalHint': 'En total, {n} g',
  'portion.unit': 'Unidad',
  'portion.spoon': 'Cucharada',
  'portion.teaspoon': 'Cucharadita',
  'portion.slice': 'Rebanada',
  'portion.glass': 'Vaso',
  'portion.pot': 'Envase',
  'portion.handful': 'Puñado',
  'portion.dose': 'Cazo',
  'add.category': 'Categoría',

  'state.title': 'Preparación',
  'state.raw': 'Crudo',
  'state.cooked': 'Cocido',

  'cat.all': 'Todos',
  'cat.favorites': 'Favoritos',
  'fav.added': '{name} en favoritos — toca su nombre para añadirlo al diario',
  'fav.removed': '{name} eliminado de favoritos',
  'fav.empty':
    'Todavía no hay favoritos. La estrella fija un alimento arriba de la lista; no lo añade al diario — para eso, toca su nombre.',
  'cat.mine': 'Mis alimentos',
  'cat.protein': 'Proteínas',
  'cat.carbs': 'Cereales',
  'cat.dairy': 'Lácteos',
  'cat.fruit': 'Frutas',
  'cat.veg': 'Verduras',
  'cat.fat': 'Grasas',
  'cat.drink': 'Bebidas',
  'cat.snack': 'Snacks',
  'cat.dish': 'Platos',
  'cat.supplement': 'Suplementos',

  'weight.title': 'Seguimiento del peso',
  'weight.current': 'Peso actual',
  'weight.log': 'Registrar',
  'weight.input': 'Peso (kg)',
  'weight.empty': 'Ningún pesaje registrado.',
  'photo.title': 'Fotos de progreso',
  'photo.empty': 'Ninguna foto por ahora.',
  'photo.add': 'Añadir una foto',
  'photo.take': 'Tomar una foto',
  'photo.retake': 'Repetir',
  'photo.saved': 'Foto guardada',
  'photo.deleted': 'Foto eliminada',
  'photo.deleteConfirm': '¿Eliminar esta foto?',
  'photo.error': 'No se pudo guardar esta foto',
  'weight.bmi': 'IMC',
  'weight.history': 'Historial de peso',
  'weight.syncProfile': 'El perfil usa siempre el último pesaje.',
  'weight.trend': 'Evolución',
  'chart.range1m': '1 M',
  'chart.range3m': '3 M',
  'chart.range6m': '6 M',
  'chart.range1y': '1 A',
  'chart.rangeAll': 'Todo',
  'chart.notEnough': 'No hay suficientes datos en este período.',
  'measure.title': 'Medidas corporales',
  'measure.waist': 'Cintura',
  'measure.hips': 'Cadera',
  'measure.chest': 'Pecho',
  'measure.log': 'Añadir nueva medida',
  'measure.logged': 'Medidas guardadas',
  'measure.empty': 'No hay medidas registradas.',
  'measure.history': 'Historial de medidas',
  'history.title': 'Historial',
  'history.open': 'Ver historial',

  'bmi.under': 'Peso insuficiente',
  'bmi.normal': 'Peso normal',
  'bmi.over': 'Sobrepeso',
  'bmi.obese': 'Obesidad',
  'bmi.morbid': 'Obesidad mórbida',

  'account.title': 'Cuenta',
  'account.loading': 'Cargando…',
  'account.connectedAs': 'Conectado como',
  'account.signOut': 'Cerrar sesión',
  'account.signIn': 'Iniciar sesión',
  'account.signUp': 'Crear cuenta',
  'account.email': 'Correo electrónico',
  'account.password': 'Contraseña',
  'account.rememberMe': 'Recordarme',
  'account.continueWithGoogle': 'Continuar con Google',
  'account.confirmEmail': 'Revisa tu correo para confirmar tu cuenta.',
  'account.greeting': '¡Qué bueno verte de nuevo, {name}!',
  'account.skip': 'Continuar sin cuenta',
  'account.syncError': 'Fallo de sincronización — se reintentará en el próximo cambio.',
  'profile.title': 'Perfil y objetivos',
  'profile.displayName': '¿Cómo prefieres que te llamemos?',
  'profile.height': 'Altura (cm)',
  'profile.weight': 'Peso (kg)',
  'profile.age': 'Edad',
  'profile.sex': 'Sexo',
  'profile.male': 'Hombre',
  'profile.female': 'Mujer',
  'profile.activity': 'Nivel de actividad',
  'profile.goal': 'Objetivo',
  'profile.targets': 'Objetivos diarios',
  'profile.preset': 'Programa',
  'preset.balanced': 'Equilibrado',
  'preset.highprotein': 'Alto en proteína',
  'preset.lowcarb': 'Bajo en carbohidratos',
  'preset.keto': 'Cetogénico',
  'preset.endurance': 'Resistencia',
  'preset.custom': 'Personalizado',
  'profile.carbsAuto': 'Los carbohidratos completan automáticamente el objetivo calórico.',
  'day.macros': 'Macros del día',
  'day.custom': 'personalizadas',
  'day.reset': 'Volver a las macros del perfil',
  'day.followsProfile': 'Este día sigue el perfil. Mover un control solo cambia este día.',
  'day.noteTitle': 'Nota del día',
  'day.notePlaceholder': 'Comida saltada, entrenamiento, viaje…',
  'day.noteSave': 'Guardar',
  'day.noteDelete': 'Eliminar la nota',
  'day.noteHoldHint': 'Truco: mantener el dedo pulsado en un día lo abre directamente en el diario.',
  'calendar.streakCalories': 'Calorías',
  'calendar.streakMacros': 'Macros',
  'profile.dayMacrosOpen': 'Abrir las macros del día en el diario',
  'profile.dayMacrosOpenHint':
    'El panel queda desplegado en todos los días en vez de estar plegado tras la flecha.',
  'profile.warnMacros':
    'Las proteínas y las grasas suman {n} kcal más que el objetivo: los carbohidratos se quedan a cero. Baja un control o sube el objetivo para que vuelvan a subir.',
  'profile.bmr': 'Metabolismo basal',
  'profile.tdee': 'Gasto estimado',
  'profile.adjustment': 'Ajuste',
  'profile.requested': 'Objetivo solicitado',
  'profile.warnBmr':
    'Objetivo solicitado: {req} kcal, por debajo del metabolismo basal. El valor retenido se eleva a {n} kcal; mientras siga por debajo de ese suelo, cambiar el peso, la edad, la actividad o el objetivo solo modifica el valor solicitado.',
  'profile.language': 'Idioma',
  'profile.data': 'Datos',
  'profile.export': 'Exportar (JSON)',
  'profile.import': 'Importar',
  'profile.reset': 'Borrar todo',
  'profile.resetConfirm': '¿Borrar definitivamente todos tus datos?',
  'profile.importError': 'Archivo no válido',
  'profile.imported': 'Datos importados',
  'profile.offline': 'Todos tus datos se quedan en este dispositivo.',
  'profile.tutorials': 'Tutoriales automáticos',
  'profile.tutorialsHint':
    'Muestra un recorrido guiado la primera vez que entras en cada pestaña. El botón "?" de la cabecera sigue disponible siempre.',

  'act.sedentary': 'Sedentario (oficina)',
  'act.light': 'Ligero (1-3× / semana)',
  'act.moderate': 'Moderado (3-5× / semana)',
  'act.active': 'Muy activo (6-7× / semana)',
  'act.athlete': 'Atleta (2× / día)',

  'goal.loss150': 'Pérdida: 1,50 kg / semana',
  'goal.loss100': 'Pérdida: 1,00 kg / semana',
  'goal.loss075': 'Pérdida: 0,75 kg / semana',
  'goal.loss050': 'Pérdida: 0,50 kg / semana',
  'goal.loss025': 'Pérdida: 0,25 kg / semana',
  'goal.maintain': 'Mantenimiento del peso',
  'goal.gain025': 'Ganancia: +0,25 kg / semana',
  'goal.gain050': 'Ganancia: +0,50 kg / semana',

  'onboarding.welcome': 'Bienvenido a FitnessFufs',
  'onboarding.intro':
    'Con unos pocos datos calculamos tus objetivos diarios. Todo se guarda en tu móvil.',
  'onboarding.start': 'Empezar',
  'onboarding.skip': 'Más tarde',

  'tour.help': 'Ayuda',
  'tour.skip': 'Omitir',
  'tour.next': 'Siguiente',
  'tour.done': 'Listo',
  'tour.calendar.1':
    'Toca un día para seleccionarlo; mantén pulsado para abrir su diario directamente. Desliza horizontalmente para cambiar de mes.',
  'tour.calendar.2':
    'Cada barra muestra la desviación respecto a tu objetivo: verde si está cerca, roja si está lejos, en ambos sentidos.',
  'tour.diary.1':
    'Cambia de día con las flechas, o desliza con dos dedos para no interferir con el desplazamiento de la página.',
  'tour.diary.2': 'Tu progreso calórico del día, actualizado con cada alimento añadido.',
  'tour.diary.3': 'Añade una nota libre a este día — entrenamiento, viaje, comida saltada.',
  'tour.add.1': 'Busca un alimento, escanea un código de barras o crea el tuyo.',
  'tour.add.2': 'Compara alimentos para alcanzar una cantidad objetivo de un nutriente.',
  'tour.weight.1':
    'La evolución de tu peso, con un selector de periodo para ver las últimas semanas o todo el historial.',
  'tour.weight.2':
    'Tu IMC, ubicado en una escala de colores para ver de un vistazo dónde te encuentras.',
  'tour.fridge.1': 'Busca un alimento y tócalo para añadirlo a la nevera con su peso.',
  'tour.fridge.2': 'Ajusta el peso disponible en cualquier momento, o quita un alimento una vez usado.',
  'tour.profile.1': 'Aquí puedes desactivar o reactivar el inicio automático de los tutoriales.',
  'tour.addCustom.1': 'El nombre del alimento, tal como aparecerá en tus búsquedas.',
  'tour.addCustom.2':
    'Compónlo a partir de alimentos ya guardados: la receta calcula automáticamente las macros y micros proporcionales.',
  'tour.addCustom.3':
    'Indica las calorías, o déjalas en blanco: se recalcularán a partir de las macros indicadas más abajo.',
  'tour.addTarget.1':
    'Compara los alimentos de una categoría concreta, o de todas las categorías a la vez.',
  'tour.addTarget.2': 'Elige el nutriente objetivo y la cantidad a alcanzar.',
  'tour.addTarget.3':
    'Cada resultado indica el peso necesario para alcanzar tu objetivo; tócalo para añadirlo directamente al diario.',

  'common.save': 'Guardar',
  'common.cancel': 'Cancelar',
  'common.delete': 'Eliminar',
  'common.close': 'Cerrar',
  'common.deficit': 'Déficit',
  'common.surplus': 'Superávit',
  'common.none': 'Ninguno',
  'common.install': 'Instalar la aplicación',
  'common.update': 'Actualización disponible',
  'common.refresh': 'Recargar',
  'common.left': 'restante',
}

const en: Dict = {
  'app.name': 'FitnessFufs',
  'app.tagline': 'Calories, macros and weight',

  'nav.diary': 'Diary',
  'nav.add': 'Add',
  'nav.weight': 'Tracking',
  'nav.profile': 'Profile',
  'nav.calendar': 'Calendar',
  'nav.fridge': 'Fridge',

  'meal.breakfast': 'Breakfast',
  'meal.lunch': 'Lunch',
  'meal.dinner': 'Dinner',
  'meal.snack': 'Snack',

  'macro.kcal': 'Calories',
  'macro.protein': 'Protein',
  'macro.carbs': 'Carbs',
  'macro.fat': 'Fat',
  'macro.protein.short': 'P',
  'macro.carbs.short': 'C',
  'macro.fat.short': 'F',
  'macro.fiber': 'Fibre',
  'macro.fiber.short': 'Fb',

  'micros.title': 'Micronutrients',
  'micros.none': 'No micronutrient data for this food.',
  'micros.partial': "Based on {n}% of today's calories — not every food has data.",
  'micros.reference': 'of reference intake',
  'micro.sodium': 'Sodium',
  'micro.potassium': 'Potassium',
  'micro.calcium': 'Calcium',
  'micro.iron': 'Iron',
  'micro.magnesium': 'Magnesium',
  'micro.zinc': 'Zinc',
  'micro.vitaminC': 'Vitamin C',
  'micro.vitaminD': 'Vitamin D',
  'micro.vitaminB12': 'Vitamin B12',

  'diary.remaining': 'Remaining',
  'diary.over': 'Over',
  'diary.eaten': 'Eaten',
  'diary.goal': 'Goal',
  'diary.today': 'Today',
  'diary.backToToday': 'Back to today',
  'diary.prevDay': 'Previous day',
  'diary.nextDay': 'Next day',
  'diary.emptyMeal': 'Nothing logged here',
  'diary.addFood': 'Add food',
  'diary.moveMealUp': 'Move up',
  'diary.moveMealDown': 'Move down',
  'diary.renameMeal': 'Rename this meal',
  'diary.markMealDone': 'Mark this meal as done',
  'diary.addMeal': 'Add a meal',
  'diary.newMealPlaceholder': 'Meal name',
  'diary.noProfile': 'Complete your profile to see your targets.',
  'diary.copyPrev': 'Copy previous day',
  'diary.copied': 'Previous day copied',
  'diary.nothingToCopy': 'Nothing to copy from the previous day',

  'add.title': 'Add food',
  'add.search': 'Search a food…',
  'add.noResults': 'No food found',
  'add.createFrom': 'Create “{q}”',
  'add.per100': 'Per 100 g',
  'add.quantity': 'Amount (g)',
  'add.meal': 'Meal',
  'add.confirm': 'Add to diary',
  'add.added': 'Added to diary',
  'add.custom': 'Custom food',
  'add.customName': 'Food name',
  'add.customHint': 'Values per 100 g',
  'add.customServing': 'Usual serving (g)',
  'add.servingHint': 'Usual serving: {n} g',
  'add.kcalMismatch': 'Calories recomputed from macros: {n} kcal',
  'add.kcalWarn':
    "The calories entered ({entered} kcal) don't match those calculated from the macros ({computed} kcal). Leave the field empty to use the calculated value automatically.",

  'recipe.title': 'Recipe',
  'recipe.add': 'Add a recipe',
  'recipe.subtitle': 'Combine foods already in the app',
  'recipe.empty': 'Add foods to build the recipe.',
  'recipe.ingredients': 'Ingredients',
  'recipe.totalWeight': 'Total weight: {n} g',
  'recipe.per100Hint': 'Values recalculated per 100 g of this mix.',
  'recipe.apply': 'Use these values',
  'recipe.of': 'of',

  'fridge.hint': 'The foods you have on hand, to put together meal suggestions around your remaining macros.',
  'fridge.empty': 'Nothing here yet. Search for a food above to add it.',
  'fridge.section.fridge': 'Fridge',
  'fridge.section.pantry': 'Pantry',
  'fridge.section.freezer': 'Freezer',
  'fridge.addTo': 'Add — {section}',

  'chat.title': 'Nutrition assistant',
  'chat.placeholder': 'Type a message…',
  'chat.send': 'Send',
  'chat.empty': 'Ask for a meal idea based on what is in the fridge and your remaining macros.',
  'chat.clear': 'Clear the conversation',
  'chat.clearConfirm': 'Permanently clear this conversation?',
  'chat.noKey': 'Add your Anthropic API key in the profile to enable the assistant.',
  'chat.capped': 'Estimated monthly cap reached ({cap} $). The assistant picks back up next month.',
  'chat.error': 'Could not reach the assistant. Check your connection or your API key.',
  'chat.thinking': 'The assistant is thinking…',
  'chat.suggest.plan': "Give me a meal plan for today with what's in the fridge.",
  'chat.suggest.remaining': 'What do I still need to eat today to hit my macros?',
  'chat.suggest.quick': 'A quick meal idea with the fridge?',
  'chat.sendToDiary': 'Send to diary',
  'chat.schedule': 'Schedule',
  'chat.pickDate': 'Pick a date',
  'chat.mealScheduled': 'Added to the diary for {date}',
  'chat.mealScheduledLabel': '{meal} for {date} scheduled',
  'chat.mealAdded': 'Meal added to the diary',
  'chat.createFood': 'Create',
  'chat.foodAdded': 'Food created and added to the diary',
  'chat.similarFound': '"{name}" looks like:',
  'chat.createInstead': 'Create a new food',
  'chat.removeIngredient': 'Remove this ingredient',
  'profile.apiKey': 'Anthropic API key',
  'profile.apiKeyHint':
    'Used only on this device, sent directly to Anthropic for the nutrition assistant. Never included in the export.',
  'profile.usagePeriod': '{pct}% used since {date}',
  'profile.usageHint':
    'Estimated from the tokens billed by the API, for reference only — not a substitute for a real cap set at console.anthropic.com. Resets one month after this period started.',
  'add.deleteFood': 'Delete this food',
  'add.editFood': 'Edit this food',
  'add.updated': 'Values updated',

  'target.title': 'Nutrition target',
  'target.subtitle': 'Compare the foods in a category',
  'target.category': 'Food category',
  'target.nutrient': 'Target nutrient',
  'target.quantity': 'Target amount',
  'target.goal': 'Target: {qty} {unit} of {nutrient}',
  'target.empty': 'No food in this category provides this nutrient.',

  'scan.title': 'Scan a barcode',
  'scan.hint': 'Frame the product barcode',
  'scan.denied': 'Camera access denied. Allow it in your browser settings.',
  'scan.unavailable': 'No camera available on this device.',
  'scan.notFound': 'Product unknown to Open Food Facts. Create it manually.',
  'scan.failed': 'Lookup failed — check your connection.',
  'off.search': 'Search Open Food Facts',
  'off.searching': 'Searching…',
  'off.results': 'Products found: {n}',
  'off.none': 'No product found on Open Food Facts.',
  'off.error': 'Open Food Facts is unreachable. Check your connection and try again.',
  'off.source': 'Open Food Facts data, contributed by its users — check the packaging.',

  'add.unit': 'Unit',
  'add.results': 'Foods: {n}',
  'add.totalHint': '{n} g in total',
  'portion.unit': 'Unit',
  'portion.spoon': 'Tablespoon',
  'portion.teaspoon': 'Teaspoon',
  'portion.slice': 'Slice',
  'portion.glass': 'Glass',
  'portion.pot': 'Pot',
  'portion.handful': 'Handful',
  'portion.dose': 'Scoop',
  'add.category': 'Category',

  'state.title': 'Preparation',
  'state.raw': 'Raw',
  'state.cooked': 'Cooked',

  'cat.all': 'All',
  'cat.favorites': 'Favourites',
  'fav.added': '{name} pinned to favourites — tap its name to add it to the diary',
  'fav.removed': '{name} removed from favourites',
  'fav.empty':
    'No favourites yet. The star pins a food to the top of the list; it does not add it to the diary — tap its name for that.',
  'cat.mine': 'My foods',
  'cat.protein': 'Protein',
  'cat.carbs': 'Grains',
  'cat.dairy': 'Dairy',
  'cat.fruit': 'Fruit',
  'cat.veg': 'Vegetables',
  'cat.fat': 'Fats',
  'cat.drink': 'Drinks',
  'cat.snack': 'Snacks',
  'cat.dish': 'Dishes',
  'cat.supplement': 'Supplements',

  'weight.title': 'Weight tracking',
  'weight.current': 'Current weight',
  'weight.log': 'Log',
  'weight.input': 'Weight (kg)',
  'weight.empty': 'No weigh-in recorded yet.',
  'photo.title': 'Progress photos',
  'photo.empty': 'No photo yet.',
  'photo.add': 'Add a photo',
  'photo.take': 'Take a photo',
  'photo.retake': 'Retake',
  'photo.saved': 'Photo saved',
  'photo.deleted': 'Photo deleted',
  'photo.deleteConfirm': 'Delete this photo?',
  'photo.error': 'Could not save this photo',
  'weight.bmi': 'BMI',
  'weight.history': 'Weight history',
  'weight.syncProfile': 'The profile always uses your latest weigh-in.',
  'weight.trend': 'Trend',
  'chart.range1m': '1M',
  'chart.range3m': '3M',
  'chart.range6m': '6M',
  'chart.range1y': '1Y',
  'chart.rangeAll': 'All',
  'chart.notEnough': 'Not enough data for this period.',
  'measure.title': 'Body measurements',
  'measure.waist': 'Waist',
  'measure.hips': 'Hips',
  'measure.chest': 'Chest',
  'measure.log': 'Add new measurement',
  'measure.logged': 'Measurements saved',
  'measure.empty': 'No measurements logged yet.',
  'measure.history': 'Measurement history',
  'history.title': 'History',
  'history.open': 'View history',

  'bmi.under': 'Underweight',
  'bmi.normal': 'Healthy weight',
  'bmi.over': 'Overweight',
  'bmi.obese': 'Obesity',
  'bmi.morbid': 'Morbid obesity',

  'account.title': 'Account',
  'account.loading': 'Loading…',
  'account.connectedAs': 'Signed in as',
  'account.signOut': 'Sign out',
  'account.signIn': 'Sign in',
  'account.signUp': 'Create account',
  'account.email': 'Email',
  'account.password': 'Password',
  'account.rememberMe': 'Remember me',
  'account.continueWithGoogle': 'Continue with Google',
  'account.confirmEmail': 'Check your email to confirm your account.',
  'account.greeting': 'Great to see you again, {name}!',
  'account.skip': 'Continue without an account',
  'account.syncError': 'Sync failed — will retry on the next change.',
  'profile.title': 'Profile and targets',
  'profile.displayName': 'What should we call you?',
  'profile.height': 'Height (cm)',
  'profile.weight': 'Weight (kg)',
  'profile.age': 'Age',
  'profile.sex': 'Sex',
  'profile.male': 'Male',
  'profile.female': 'Female',
  'profile.activity': 'Activity level',
  'profile.goal': 'Goal',
  'profile.targets': 'Daily targets',
  'profile.preset': 'Preset',
  'preset.balanced': 'Balanced',
  'preset.highprotein': 'High protein',
  'preset.lowcarb': 'Low carb',
  'preset.keto': 'Keto',
  'preset.endurance': 'Endurance',
  'preset.custom': 'Custom',
  'profile.carbsAuto': 'Carbs automatically fill the rest of the calorie target.',
  'day.macros': 'Macros for the day',
  'day.custom': 'custom',
  'day.reset': 'Back to profile macros',
  'day.followsProfile': 'This day follows the profile. Moving a slider changes this day only.',
  'day.noteTitle': "Day's note",
  'day.notePlaceholder': 'Skipped meal, workout, travel…',
  'day.noteSave': 'Save',
  'day.noteDelete': 'Delete note',
  'day.noteHoldHint': 'Tip: press and hold a day to open it directly in the diary.',
  'calendar.streakCalories': 'Calories',
  'calendar.streakMacros': 'Macros',
  'profile.dayMacrosOpen': 'Open the day macros in the diary',
  'profile.dayMacrosOpenHint':
    'The panel stays open on every day instead of folded behind its arrow.',
  'profile.warnMacros':
    'Protein and fat come to {n} kcal more than the target: carbs stay at zero. Lower a slider or raise the target to bring them back.',
  'profile.bmr': 'Basal metabolic rate',
  'profile.tdee': 'Estimated expenditure',
  'profile.adjustment': 'Adjustment',
  'profile.requested': 'Requested target',
  'profile.warnBmr':
    'Requested target: {req} kcal, below your basal rate. The value used is raised to {n} kcal; while it stays under that floor, changing weight, age, activity or goal only moves the requested value.',
  'profile.language': 'Language',
  'profile.data': 'Data',
  'profile.export': 'Export (JSON)',
  'profile.import': 'Import',
  'profile.reset': 'Erase everything',
  'profile.resetConfirm': 'Permanently erase all your data?',
  'profile.importError': 'Invalid file',
  'profile.imported': 'Data imported',
  'profile.offline': 'All your data stays on this device.',
  'profile.tutorials': 'Automatic tutorials',
  'profile.tutorialsHint':
    'Shows a guided tour the first time you open each tab. The "?" button in the header always stays available.',

  'act.sedentary': 'Sedentary (desk job)',
  'act.light': 'Light (1-3× / week)',
  'act.moderate': 'Moderate (3-5× / week)',
  'act.active': 'Very active (6-7× / week)',
  'act.athlete': 'Athlete (2× / day)',

  'goal.loss150': 'Loss: 1.50 kg / week',
  'goal.loss100': 'Loss: 1.00 kg / week',
  'goal.loss075': 'Loss: 0.75 kg / week',
  'goal.loss050': 'Loss: 0.50 kg / week',
  'goal.loss025': 'Loss: 0.25 kg / week',
  'goal.maintain': 'Maintain weight',
  'goal.gain025': 'Gain: +0.25 kg / week',
  'goal.gain050': 'Gain: +0.50 kg / week',

  'onboarding.welcome': 'Welcome to FitnessFufs',
  'onboarding.intro':
    'A few details are enough to compute your daily targets. Everything stays on your phone.',
  'onboarding.start': 'Get started',
  'onboarding.skip': 'Later',

  'tour.help': 'Help',
  'tour.skip': 'Skip',
  'tour.next': 'Next',
  'tour.done': 'Done',
  'tour.calendar.1':
    'Tap a day to select it; hold it down to jump straight into its diary. Swipe sideways to change month.',
  'tour.calendar.2':
    'Each bar shows how far you are from your target: green if close, red if far, in either direction.',
  'tour.diary.1':
    "Change day with the arrows, or swipe with two fingers so it doesn't clash with scrolling the page.",
  'tour.diary.2': 'Your calorie progress for the day, updated with every food you add.',
  'tour.diary.3': 'Add a free-form note to this day — a workout, a trip, a skipped meal.',
  'tour.add.1': 'Search for a food, scan a barcode, or create your own.',
  'tour.add.2': 'Compare foods to reach a target amount of a nutrient.',
  'tour.weight.1':
    'Your weight over time, with a period picker to zoom into recent weeks or the full history.',
  'tour.weight.2':
    'Your BMI, placed on a colour-coded gauge so you can see where you stand at a glance.',
  'tour.fridge.1': 'Search for a food and tap it to add it to the fridge with its weight.',
  'tour.fridge.2': 'Adjust the weight on hand at any time, or remove a food once you use it.',
  'tour.profile.1': 'Here you can turn the automatic tutorials off or back on.',
  'tour.addCustom.1': "The food's name, exactly as it will appear in your searches.",
  'tour.addCustom.2':
    "Build it from foods you've already saved: the recipe automatically works out the proportional macros and micros.",
  'tour.addCustom.3': "Enter the calories, or leave them blank: they'll be recalculated from the macros below.",
  'tour.addTarget.1': 'Compare foods within one category, or across every category at once.',
  'tour.addTarget.2': 'Choose the target nutrient and the amount to reach.',
  'tour.addTarget.3':
    'Each result shows the weight needed to hit your target; tap it to log it straight into your diary.',

  'common.save': 'Save',
  'common.cancel': 'Cancel',
  'common.delete': 'Delete',
  'common.close': 'Close',
  'common.deficit': 'Deficit',
  'common.surplus': 'Surplus',
  'common.none': 'None',
  'common.install': 'Install the app',
  'common.update': 'Update available',
  'common.refresh': 'Reload',
  'common.left': 'left',
}

const it: Dict = {
  'app.name': 'FitnessFufs',
  'app.tagline': 'Calorie, macro e peso',

  'nav.diary': 'Diario',
  'nav.add': 'Aggiungi',
  'nav.weight': 'Monitoraggio',
  'nav.profile': 'Profilo',
  'nav.calendar': 'Calendario',
  'nav.fridge': 'Frigo',

  'meal.breakfast': 'Colazione',
  'meal.lunch': 'Pranzo',
  'meal.dinner': 'Cena',
  'meal.snack': 'Spuntino',

  'macro.kcal': 'Calorie',
  'macro.protein': 'Proteine',
  'macro.carbs': 'Carboidrati',
  'macro.fat': 'Grassi',
  'macro.protein.short': 'P',
  'macro.carbs.short': 'C',
  'macro.fat.short': 'G',
  'macro.fiber': 'Fibre',
  'macro.fiber.short': 'F',

  'micros.title': 'Micronutrienti',
  'micros.none': 'Micronutrienti non disponibili per questo alimento.',
  'micros.partial': 'Calcolato sul {n} % delle calorie di oggi: non tutti gli alimenti sono coperti.',
  'micros.reference': 'dei valori di riferimento',
  'micro.sodium': 'Sodio',
  'micro.potassium': 'Potassio',
  'micro.calcium': 'Calcio',
  'micro.iron': 'Ferro',
  'micro.magnesium': 'Magnesio',
  'micro.zinc': 'Zinco',
  'micro.vitaminC': 'Vitamina C',
  'micro.vitaminD': 'Vitamina D',
  'micro.vitaminB12': 'Vitamina B12',

  'diary.remaining': 'Rimanenti',
  'diary.over': 'Superato',
  'diary.eaten': 'Consumate',
  'diary.goal': 'Obiettivo',
  'diary.today': 'Oggi',
  'diary.backToToday': 'Torna a oggi',
  'diary.prevDay': 'Giorno precedente',
  'diary.nextDay': 'Giorno successivo',
  'diary.emptyMeal': 'Niente in questo pasto',
  'diary.addFood': 'Aggiungi alimento',
  'diary.moveMealUp': 'Sposta in alto',
  'diary.moveMealDown': 'Sposta in basso',
  'diary.renameMeal': 'Rinomina questo pasto',
  'diary.markMealDone': 'Segna questo pasto come completato',
  'diary.addMeal': 'Aggiungi pasto',
  'diary.newMealPlaceholder': 'Nome del pasto',
  'diary.noProfile': 'Completa il profilo per vedere i tuoi obiettivi.',
  'diary.copyPrev': 'Copia il giorno prima',
  'diary.copied': 'Diario del giorno prima copiato',
  'diary.nothingToCopy': 'Niente da copiare dal giorno prima',

  'add.title': 'Aggiungi alimento',
  'add.search': 'Cerca un alimento…',
  'add.noResults': 'Nessun alimento trovato',
  'add.createFrom': 'Crea «{q}»',
  'add.per100': 'Per 100 g',
  'add.quantity': 'Quantità (g)',
  'add.meal': 'Pasto',
  'add.confirm': 'Aggiungi al diario',
  'add.added': 'Aggiunto al diario',
  'add.custom': 'Alimento personalizzato',
  'add.customName': "Nome dell'alimento",
  'add.customHint': 'Valori per 100 g',
  'add.customServing': 'Porzione abituale (g)',
  'add.servingHint': 'Porzione abituale: {n} g',
  'add.kcalMismatch': 'Calorie ricalcolate dai macro: {n} kcal',
  'add.kcalWarn':
    'Le calorie inserite ({entered} kcal) non corrispondono a quelle calcolate dai macro ({computed} kcal). Lascia il campo vuoto per usare il valore calcolato automaticamente.',

  'recipe.title': 'Ricetta',
  'recipe.add': 'Aggiungi una ricetta',
  'recipe.subtitle': 'Combina alimenti già registrati',
  'recipe.empty': 'Aggiungi alimenti per comporre la ricetta.',
  'recipe.ingredients': 'Ingredienti',
  'recipe.totalWeight': 'Peso totale: {n} g',
  'recipe.per100Hint': 'Valori ricalcolati per 100 g di questo mix.',
  'recipe.apply': 'Usa questi valori',
  'recipe.of': 'di',

  'fridge.hint':
    'Gli alimenti che hai a disposizione, per preparare suggerimenti di pasto in base alle macro rimanenti.',
  'fridge.empty': 'Ancora niente qui. Cerca un alimento qui sopra per aggiungerlo.',
  'fridge.section.fridge': 'Frigo',
  'fridge.section.pantry': 'Dispensa',
  'fridge.section.freezer': 'Congelatore',
  'fridge.addTo': 'Aggiungi — {section}',

  'chat.title': 'Assistente nutrizionale',
  'chat.placeholder': 'Scrivi un messaggio…',
  'chat.send': 'Invia',
  'chat.empty': "Chiedi un'idea di pasto in base a cosa c'è in frigo e alle tue macro rimanenti.",
  'chat.clear': 'Cancella la conversazione',
  'chat.clearConfirm': 'Cancellare definitivamente questa conversazione?',
  'chat.noKey': "Aggiungi la tua chiave API Anthropic nel profilo per attivare l'assistente.",
  'chat.capped': "Limite mensile stimato raggiunto ({cap} $). L'assistente riprende il mese prossimo.",
  'chat.error': "Impossibile contattare l'assistente. Controlla la connessione o la chiave API.",
  'chat.thinking': "L'assistente sta pensando…",
  'chat.suggest.plan': "Preparami un piano dei pasti di oggi con quello che c'è in frigo.",
  'chat.suggest.remaining': 'Cosa mi resta da mangiare oggi per raggiungere le mie macro?',
  'chat.suggest.quick': "Un'idea di pasto veloce con il frigo?",
  'chat.sendToDiary': 'Invia al diario',
  'chat.schedule': 'Pianifica',
  'chat.pickDate': 'Scegli una data',
  'chat.mealScheduled': 'Aggiunto al diario del {date}',
  'chat.mealScheduledLabel': '{meal} del {date} pianificato',
  'chat.mealAdded': 'Pasto aggiunto al diario',
  'chat.createFood': 'Crea',
  'chat.foodAdded': 'Alimento creato e aggiunto al diario',
  'chat.similarFound': '"{name}" assomiglia a:',
  'chat.createInstead': 'Crea un nuovo alimento',
  'chat.removeIngredient': 'Rimuovi questo ingrediente',
  'profile.apiKey': 'Chiave API Anthropic',
  'profile.apiKeyHint':
    "Usata solo su questo dispositivo, inviata direttamente ad Anthropic per l'assistente nutrizionale. Mai inclusa nell'esportazione.",
  'profile.usagePeriod': '{pct}% utilizzato dal {date}',
  'profile.usageHint':
    "Stima basata sui token fatturati dall'API, solo indicativa — non sostituisce un limite reale impostato su console.anthropic.com. Azzerato un mese dopo l'inizio di questo periodo.",
  'add.deleteFood': 'Elimina questo alimento',
  'add.editFood': 'Correggi questo alimento',
  'add.updated': 'Valori aggiornati',

  'target.title': 'Obiettivo nutrizionale',
  'target.subtitle': 'Confronta gli alimenti di una categoria',
  'target.category': 'Categoria di alimenti',
  'target.nutrient': 'Nutriente obiettivo',
  'target.quantity': 'Quantità obiettivo',
  'target.goal': 'Obiettivo: {qty} {unit} di {nutrient}',
  'target.empty': 'Nessun alimento di questa categoria fornisce questo nutriente.',

  'scan.title': 'Scansiona un codice a barre',
  'scan.hint': 'Inquadra il codice a barre del prodotto',
  'scan.denied': 'Accesso alla fotocamera negato. Autorizzalo nelle impostazioni del browser.',
  'scan.unavailable': 'Fotocamera non disponibile su questo dispositivo.',
  'scan.notFound': 'Prodotto sconosciuto a Open Food Facts. Crealo a mano.',
  'scan.failed': 'Ricerca impossibile: controlla la connessione.',
  'off.search': 'Cerca su Open Food Facts',
  'off.searching': 'Ricerca in corso…',
  'off.results': 'Prodotti trovati: {n}',
  'off.none': 'Nessun prodotto trovato su Open Food Facts.',
  'off.error': 'Open Food Facts non è raggiungibile. Controlla la connessione e riprova.',
  'off.source': 'Dati Open Food Facts, forniti dai suoi utenti — verifica sulla confezione.',

  'add.unit': 'Unità',
  'add.results': 'Alimenti: {n}',
  'add.totalHint': 'In tutto {n} g',
  'portion.unit': 'Unità',
  'portion.spoon': 'Cucchiaio',
  'portion.teaspoon': 'Cucchiaino',
  'portion.slice': 'Fetta',
  'portion.glass': 'Bicchiere',
  'portion.pot': 'Vasetto',
  'portion.handful': 'Manciata',
  'portion.dose': 'Misurino',
  'add.category': 'Categoria',

  'state.title': 'Preparazione',
  'state.raw': 'Crudo',
  'state.cooked': 'Cotto',

  'cat.all': 'Tutti',
  'cat.favorites': 'Preferiti',
  'fav.added': '{name} tra i preferiti — tocca il nome per aggiungerlo al diario',
  'fav.removed': '{name} rimosso dai preferiti',
  'fav.empty':
    "Nessun preferito. La stella fissa un alimento in cima all'elenco; non lo aggiunge al diario — per farlo, tocca il nome.",
  'cat.mine': 'I miei alimenti',
  'cat.protein': 'Proteine',
  'cat.carbs': 'Cereali',
  'cat.dairy': 'Latticini',
  'cat.fruit': 'Frutta',
  'cat.veg': 'Verdura',
  'cat.fat': 'Grassi',
  'cat.drink': 'Bevande',
  'cat.snack': 'Snack',
  'cat.dish': 'Piatti',
  'cat.supplement': 'Integratori',

  'weight.title': 'Monitoraggio del peso',
  'weight.current': 'Peso attuale',
  'weight.log': 'Registra',
  'weight.input': 'Peso (kg)',
  'weight.empty': 'Nessuna pesata registrata.',
  'photo.title': 'Foto di progresso',
  'photo.empty': 'Nessuna foto per ora.',
  'photo.add': 'Aggiungi una foto',
  'photo.take': 'Scatta una foto',
  'photo.retake': 'Rifai',
  'photo.saved': 'Foto salvata',
  'photo.deleted': 'Foto eliminata',
  'photo.deleteConfirm': 'Eliminare questa foto?',
  'photo.error': 'Impossibile salvare questa foto',
  'weight.bmi': 'IMC',
  'weight.history': 'Cronologia del peso',
  'weight.syncProfile': "Il profilo usa sempre l'ultima pesata.",
  'weight.trend': 'Andamento',
  'chart.range1m': '1 M',
  'chart.range3m': '3 M',
  'chart.range6m': '6 M',
  'chart.range1y': '1 A',
  'chart.rangeAll': 'Tutto',
  'chart.notEnough': 'Dati insufficienti per questo periodo.',
  'measure.title': 'Misure corporee',
  'measure.waist': 'Vita',
  'measure.hips': 'Fianchi',
  'measure.chest': 'Torace',
  'measure.log': 'Aggiungi nuova misura',
  'measure.logged': 'Misure salvate',
  'measure.empty': 'Nessuna misura registrata.',
  'measure.history': 'Cronologia misure',
  'history.title': 'Cronologia',
  'history.open': 'Vedi cronologia',

  'bmi.under': 'Sottopeso',
  'bmi.normal': 'Normopeso',
  'bmi.over': 'Sovrappeso',
  'bmi.obese': 'Obesità',
  'bmi.morbid': 'Obesità grave',

  'account.title': 'Account',
  'account.loading': 'Caricamento…',
  'account.connectedAs': 'Connesso come',
  'account.signOut': 'Disconnetti',
  'account.signIn': 'Accedi',
  'account.signUp': 'Crea account',
  'account.email': 'E-mail',
  'account.password': 'Password',
  'account.rememberMe': 'Ricordami',
  'account.continueWithGoogle': 'Continua con Google',
  'account.confirmEmail': "Controlla l'e-mail per confermare il tuo account.",
  'account.greeting': 'Che bello rivederti, {name}!',
  'account.skip': 'Continua senza account',
  'account.syncError': 'Sincronizzazione fallita — nuovo tentativo alla prossima modifica.',
  'profile.title': 'Profilo e obiettivi',
  'profile.displayName': 'Come vuoi essere chiamato?',
  'profile.height': 'Altezza (cm)',
  'profile.weight': 'Peso (kg)',
  'profile.age': 'Età',
  'profile.sex': 'Sesso',
  'profile.male': 'Uomo',
  'profile.female': 'Donna',
  'profile.activity': 'Livello di attività',
  'profile.goal': 'Obiettivo',
  'profile.targets': 'Obiettivi giornalieri',
  'profile.preset': 'Programma',
  'preset.balanced': 'Equilibrato',
  'preset.highprotein': 'Ricco di proteine',
  'preset.lowcarb': 'Pochi carboidrati',
  'preset.keto': 'Chetogenico',
  'preset.endurance': 'Resistenza',
  'preset.custom': 'Personalizzato',
  'profile.carbsAuto': "I carboidrati completano automaticamente l'obiettivo calorico.",
  'day.macros': 'Macro del giorno',
  'day.custom': 'personalizzate',
  'day.reset': 'Tornare alle macro del profilo',
  'day.followsProfile': 'Questa giornata segue il profilo. Spostare un cursore cambia solo questo giorno.',
  'day.noteTitle': 'Nota del giorno',
  'day.notePlaceholder': 'Pasto saltato, allenamento, viaggio…',
  'day.noteSave': 'Salva',
  'day.noteDelete': 'Elimina la nota',
  'day.noteHoldHint': 'Suggerimento: tenere premuto un giorno lo apre direttamente nel diario.',
  'calendar.streakCalories': 'Calorie',
  'calendar.streakMacros': 'Macro',
  'profile.dayMacrosOpen': 'Aprire le macro del giorno nel diario',
  'profile.dayMacrosOpenHint':
    "Il pannello resta aperto ogni giorno invece di restare piegato dietro la freccia.",
  'profile.warnMacros':
    "Proteine e grassi superano di {n} kcal l'obiettivo: i carboidrati restano a zero. Abbassa un cursore o alza l'obiettivo per farli risalire.",
  'profile.bmr': 'Metabolismo basale',
  'profile.tdee': 'Dispendio stimato',
  'profile.adjustment': 'Aggiustamento',
  'profile.requested': 'Obiettivo richiesto',
  'profile.warnBmr':
    "Obiettivo richiesto: {req} kcal, sotto il metabolismo basale. Il valore adottato è portato a {n} kcal; finché resta sotto questa soglia, cambiare peso, età, attività o obiettivo modifica solo il valore richiesto.",
  'profile.language': 'Lingua',
  'profile.data': 'Dati',
  'profile.export': 'Esporta (JSON)',
  'profile.import': 'Importa',
  'profile.reset': 'Cancella tutto',
  'profile.resetConfirm': 'Cancellare definitivamente tutti i tuoi dati?',
  'profile.importError': 'File non valido',
  'profile.imported': 'Dati importati',
  'profile.offline': 'Tutti i dati restano su questo dispositivo.',
  'profile.tutorials': 'Tutorial automatici',
  'profile.tutorialsHint':
    "Mostra un tour guidato la prima volta che apri ogni scheda. Il pulsante « ? » nell'intestazione resta sempre disponibile.",

  'act.sedentary': 'Sedentario (ufficio)',
  'act.light': 'Leggero (1-3× / settimana)',
  'act.moderate': 'Moderato (3-5× / settimana)',
  'act.active': 'Molto attivo (6-7× / settimana)',
  'act.athlete': 'Atleta (2× / giorno)',

  'goal.loss150': 'Perdita: 1,50 kg / settimana',
  'goal.loss100': 'Perdita: 1,00 kg / settimana',
  'goal.loss075': 'Perdita: 0,75 kg / settimana',
  'goal.loss050': 'Perdita: 0,50 kg / settimana',
  'goal.loss025': 'Perdita: 0,25 kg / settimana',
  'goal.maintain': 'Mantenimento del peso',
  'goal.gain025': 'Aumento: +0,25 kg / settimana',
  'goal.gain050': 'Aumento: +0,50 kg / settimana',

  'onboarding.welcome': 'Benvenuto su FitnessFufs',
  'onboarding.intro':
    'Bastano poche informazioni per calcolare i tuoi obiettivi giornalieri. Tutto resta sul tuo telefono.',
  'onboarding.start': 'Inizia',
  'onboarding.skip': 'Più tardi',

  'tour.help': 'Aiuto',
  'tour.skip': 'Salta',
  'tour.next': 'Avanti',
  'tour.done': 'Fatto',
  'tour.calendar.1':
    'Tocca un giorno per selezionarlo; tienilo premuto per aprire subito il suo diario. Scorri orizzontalmente per cambiare mese.',
  'tour.calendar.2':
    'Ogni barra mostra lo scostamento dal tuo obiettivo: verde se vicino, rossa se lontano, in entrambe le direzioni.',
  'tour.diary.1':
    'Cambia giorno con le frecce, oppure scorri con due dita per non interferire con lo scorrimento della pagina.',
  'tour.diary.2': 'Il tuo progresso calorico del giorno, aggiornato a ogni alimento aggiunto.',
  'tour.diary.3': 'Aggiungi una nota libera a questo giorno — allenamento, viaggio, pasto saltato.',
  'tour.add.1': 'Cerca un alimento, scansiona un codice a barre o crea il tuo.',
  'tour.add.2': 'Confronta gli alimenti per raggiungere una quantità obiettivo di un nutriente.',
  'tour.weight.1':
    "L'andamento del tuo peso, con un selettore di periodo per le ultime settimane o tutta la cronologia.",
  'tour.weight.2':
    "Il tuo IMC, posizionato su una scala colorata per vedere a colpo d'occhio dove ti trovi.",
  'tour.fridge.1': 'Cerca un alimento e toccalo per aggiungerlo al frigo con il suo peso.',
  'tour.fridge.2':
    'Modifica il peso disponibile in qualsiasi momento, oppure rimuovi un alimento una volta usato.',
  'tour.profile.1': "Qui puoi disattivare o riattivare l'avvio automatico dei tutorial.",
  'tour.addCustom.1': "Il nome dell'alimento, così come apparirà nelle tue ricerche.",
  'tour.addCustom.2':
    'Componilo a partire da alimenti già salvati: la ricetta calcola automaticamente macro e micronutrienti proporzionali.',
  'tour.addCustom.3':
    'Indica le calorie, oppure lasciale vuote: verranno ricalcolate dalle macro inserite più sotto.',
  'tour.addTarget.1':
    'Confronta gli alimenti di una categoria specifica, o di tutte le categorie insieme.',
  'tour.addTarget.2': 'Scegli il nutriente obiettivo e la quantità da raggiungere.',
  'tour.addTarget.3':
    'Ogni risultato mostra il peso necessario per raggiungere il tuo obiettivo; toccalo per aggiungerlo direttamente al diario.',

  'common.save': 'Salva',
  'common.cancel': 'Annulla',
  'common.delete': 'Elimina',
  'common.close': 'Chiudi',
  'common.deficit': 'Deficit',
  'common.surplus': 'Surplus',
  'common.none': 'Nessuno',
  'common.install': "Installa l'app",
  'common.update': 'Aggiornamento disponibile',
  'common.refresh': 'Ricarica',
  'common.left': 'rimasti',
}

export const TRANSLATIONS: Record<Lang, Dict> = { fr, pt, es, en, it }

export const LANGS: { id: Lang; label: string }[] = [
  { id: 'fr', label: 'FR' },
  { id: 'pt', label: 'PT' },
  { id: 'es', label: 'ES' },
  { id: 'en', label: 'EN' },
  { id: 'it', label: 'IT' },
]

/** Langue de l'appareil si elle est supportée, français sinon. */
export function detectLang(): Lang {
  const supported = LANGS.map((entry) => entry.id)
  for (const tag of navigator.languages ?? [navigator.language]) {
    const code = tag.slice(0, 2).toLowerCase() as Lang
    if (supported.includes(code)) return code
  }
  return 'fr'
}
